...
export interface UseFileUploadOptions {
  uploadUrl?: string; accept?: string; maxSizeMB?: number; multiple?: boolean;
  clearOnSuccess?: boolean; successMessage?: string; emptySelectionMessage?: string;
  onFilesSelected?: (files: File[]) => void; onUploadSuccess?: () => void;
  onUploadError?: (message: string) => void;
}
export interface UseFileUploadResult {
  file: File | null; selectedFiles: File[]; previews: string[]; isUploading: boolean;
  message: string | null; error: string | null; inputRef: React.RefObject<HTMLInputElement>;
  handleFileChange: (event: React.ChangeEvent<HTMLInputElement>) => void;
  handleUpload: () => Promise<void>; handleRemove: () => void;
}
...
export const useFileUpload = ({
  uploadUrl, accept, maxSizeMB, multiple = false, clearOnSuccess = false,
  successMessage = 'Upload successful!', emptySelectionMessage = 'Please select a file before uploading.',
  onFilesSelected, onUploadSuccess, onUploadError,
  ...
  const resetSelection = useCallback(() => {
    setSelectedFiles([]); replacePreviews([]); if (inputRef.current) inputRef.current.value = '';
  }, [replacePreviews]);
  const selectFiles = useCallback((files: File[]) => {
    const validFiles: File[] = [], errors: string[] = [];
    const candidates = multiple ? files : files.slice(0, 1);
    const maxBytes = maxSizeMB !== undefined && Number.isFinite(maxSizeMB) && maxSizeMB > 0 ? maxSizeMB * 1024 * 1024 : null;
    if (!multiple && files.length > 1) errors.push('Only one file can be selected.');
    for (const file of candidates) {
      if (maxBytes !== null && file.size > maxBytes) { errors.push(`File "${file.name}" exceeds ${maxSizeMB}MB limit.`); continue; }
      if (!isAcceptedFile(file, accept)) { errors.push(`File "${file.name}" is not an accepted file type.`); continue; }
      validFiles.push(file);
    }
    setMessage(null);
    if (validFiles.length === 0) { setError(errors.length > 0 ? errors.join(' ') : 'No valid files selected.'); resetSelection(); return; }
    const nextPreviews = validFiles.map((file) => file.type.startsWith('image/') ? URL.createObjectURL(file) : '');
    setError(errors.length > 0 ? errors.join(' ') : null); setSelectedFiles(validFiles); replacePreviews(nextPreviews); onFilesSelected?.(validFiles);
  ...
  const handleUpload = useCallback(async () => {
    ...
    setIsUploading(true); setMessage(null); setError(null);
    try {
      const formData = new FormData();
      const fieldName = multiple ? 'files' : 'file';
      for (const file of selectedFiles) formData.append(fieldName, file, file.name);
      ...
      setError(uploadErrorMessage); onUploadError?.(uploadErrorMessage); console.error('Upload error:', uploadError);
    } finally {
      if (abortControllerRef.current === controller) {
        abortControllerRef.current = null; uploadInFlightRef.current = false;
  ...
  useEffect(() => {
    ...
    return () => {
      isMountedRef.current = false; uploadInFlightRef.current = false;
      abortControllerRef.current?.abort(); abortControllerRef.current = null;
      previewsRef.current.forEach((url) => { if (url) URL.revokeObjectURL(url); }); previewsRef.current = [];
    };
  }, []);
